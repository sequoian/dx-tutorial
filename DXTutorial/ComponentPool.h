...
public:
	...
	U64 CreateComponent(Entity e)
	{
		...
		if (m_freedIds.size() > m_minFree)
		{
			...
			remapId = Combine(m_numActive, gen);
			m_remapToPool[idx] = remapId;
		}
		else
		{
			...
			// generate remap value
			remapId = Combine(m_numActive, gen);
			m_remapToPool.push_back(remapId);
		}


		// add component to pool
		m_pool.push_back(m_next);
		m_remapToId.push_back(id);
		m_numActive++;

		// set entity map
		m_entityMap.emplace(e.id, remapId);
	...
	{
		U64 remap = m_remapToPool[Idx(id)];
	...
	void DestroyComponentById(U64 id)
	{
		...
		U32 gen = Gen(id);
		U64 remapId = m_remapToPool[idxToRemap];
		...
		if (gen == remapGen)
		{
			...
			// update pool idx to id remap
			U64 swappedId = m_remapToId.back();
			m_remapToId[idxToPool] = back;
			m_remapToId.pop_back();

			// update id to pool idx remap
			m_remapToPool[Idx(swappedId)] = Combine(idxToPool, Gen(swappedId));
